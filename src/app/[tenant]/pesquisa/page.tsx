  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-6">
                ...
                {clientes.map((c, i) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center justify-between px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors ${i < clientes.length - 1 ? 'border-b border-slate-100 dark:border-slate-800' : ''}`}
                    >
                      <div>
                        <div className="text-sm font-medium text-slate-900 dark:text-slate-100">{c.nome}</div>
                        <div className="text-xs font-mono text-slate-500 dark:text-slate-500 mt-0.5">{c.contacto}</div>
                      </div>
                ...
                {todosPedidos.map(p => {
                  ...
                  return (
                    <Link
                      ...
                      className="block bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 px-4 py-3 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          {matricula && <div className="text-xs font-mono text-slate-500 dark:text-slate-500">{matricula}</div>}
                        </div>
                  ...
                  <tbody>
                    {todosPedidos.map(p => {
                      ...
                      return (
                        <tr
                          key={p.id}
                          className="border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors cursor-pointer"
                        ...
                        >
                          ...
                          <td className="px-4 py-3 font-mono text-slate-500 dark:text-slate-400">{matricula}</td>
          ...
          {clientes.length === 0 && todosPedidos.length === 0 && (
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 py-14 text-center">
              <p className="text-slate-500 text-sm">Nenhum resultado para <span className="font-medium text-slate-700 dark:text-slate-300">&ldquo;{termo}&rdquo;</span></p>
            </div>
          )}
        </div>
      )}
      ...
      {termo.length > 0 && termo.length < 2 && (
        <p className="text-sm text-slate-500 dark:text-slate-400">Escreve pelo menos 2 caracteres para pesquisar.</p>
      )}
    </div>
  )
}
