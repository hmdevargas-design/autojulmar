            ...
            {pedidos.map((pedido) => {
              ...
              return (
                <tr key={pedido.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 font-mono text-gray-500">#{pedido.numero_pedido}</td>
                  <td className="px-4 py-3">
                    <div className="font-medium text-gray-900">{cliente?.nome ?? '—'}</div>
                    <div className="text-xs text-gray-400">{cliente?.contacto}</div>
                  </td>
                  <td className="px-4 py-3 font-mono text-gray-700">{dados?.matricula ?? '—'}</td>
                  <td className="px-4 py-3">
                    {estado ? (
                      <SeletorEstado
                        pedidoId={pedido.id}
        ...
        {pedidos.length === 0 && (
          <div className="text-center py-12 text-gray-500">
            {termo || estadoFiltroId || de || ate
              ...
              : <><span>Nenhum pedido encontrado. </span><Link href={`/${slug}/pedidos/novo`} className="text-blue-600 hover:underline">Criar primeiro pedido</Link></>
            }
          </div>
        )}
      </div>
